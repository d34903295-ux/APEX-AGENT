...
def fetch_symbol_flow(symbol: str, *, timeout: float = 6.0) -> dict:
    ...
    try:  # pragma: no cover - network
        funding = parse_funding(get_json(f"{FAPI}/fapi/v1/premiumIndex?symbol={perp}", timeout=timeout))
        oi = parse_open_interest(get_json(f"{FAPI}/fapi/v1/openInterest?symbol={perp}", timeout=timeout))
        cvd = parse_cvd_slope(get_json(f"{FAPI}/fapi/v1/aggTrades?symbol={perp}&limit=1000", timeout=timeout))
